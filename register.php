<?php
	...
	if ($_REQUEST["format"] == "feed") {
		...
		print "<feed xmlns=\"http://www.w3.org/2005/Atom\">
			...
			<title>Agriget registration slots</title>
...
<?php
		if (!ENABLE_REGISTRATION) {
			...
			print "<p><form method=\"GET\" action=\"backend.php\">
				...
				<input type=\"submit\" value=\"".__("Return to Agriget")."\">
				</form>";
			return;
		}
?>
			...
			if ($is_registered) {
				...
				if (db_num_rows($result) != 1) {
					...
					$rc = $mailer->mail(["to_address" => $email,
						"subject" => "Registration information for Agriget",
						"message" => $reg_text]);
					...
					$reg_text = "Hi!\n".
						"\n".
						"New user had registered at your Agriget installation.\n".
						"\n".
						"Login: $login\n".
						"Email: $email\n";
					...
					$rc = $mailer->mail(["to_address" => REG_NOTIFY_ADDRESS,
						"subject" => "Registration notice for Agriget",
						"message" => $reg_text]);
