...
define(["dojo/_base/declare"], function (declare) {
	...
	Headlines = {
		...
		viewFull: function (event, id) {
			const row = $("RROW-" + id);
			if (row.hasClassName("marked"))
				$("slide-out").addClassName("marked");
			else
				$("slide-out").removeClassName("marked");
			if (row.hasClassName("published"))
				$("slide-out").addClassName("published");
			else
				$("slide-out").removeClassName("published");
			var content = document.getElementById('content-'+id).innerHTML;
			var slide = '<br><br><br>'+this.fullArticles[id].replace("[[CONTENT]]", content)+'<br><br><br>';
			document.getElementById('slide-out').innerHTML = " ";
			const tmp = document.createElement("div");
			tmp.innerHTML = slide;
			dojo.parser.parse(tmp);
			document.getElementById('slide-out').appendChild(tmp);
			document.getElementById('slide-out-shadow').style.visibility = 'visible';
			document.getElementById('slide-out').style.visibility = 'visible';
		},
		hideFull: function () {
			document.getElementById('slide-out').style.visibility = 'hidden';
			document.getElementById('slide-out-shadow').style.visibility = 'hidden';
			Article.setActiveHeadline(0);
		},
		click: function (event, id, in_body) {
			in_body = in_body || false;

			if (App.isCombinedMode()) {
				//console.log("in_body: " + in_body);
				...
				} else if (id != Article.getActiveHeadline()) {
					Article.setActiveHeadline(id);
					this.viewFull(event, id);
				} else {
					Article.openInNewWindow(id);
				}
			...
			if (App.isCombinedMode()) {
				...
				this.fullArticles[hl.id] = `<div class="header">
									<div class="grid-full-container">
										<div class="grid-headline title grid-full-title hlMenuAttach" onclick="return Headlines.click(event, ${hl.id});" data-article-id="${hl.id}">
											<a class="title grid-full-title" title="${hl.title}" target="_blank" rel="noopener noreferrer" href="${hl.link}">
												${hl.title}
											</a>
										</div>
				...
				if (App.getViewMode() == "magazine") {
					row = `<div class="view-magazine cdm ${row_class} ${Article.getScoreClass(hl.score)}" id="RROW-${hl.id}" data-article-id="${hl.id}" data-orig-feed-id="${hl.feed_id}" data-content="${escapeHtml(hl.content)}" data-score="${hl.score}" onmouseover="Article.mouseIn(${hl.id})" onmouseout="Article.mouseOut(${hl.id})">
							<div class="header">
								<div class="grid-container">
									<div class="grid-image">
										<div class="img_preview">
											<img class="img_preview" src="${img_preview_src}">
										</div>
									</div>
									<div class="grid-headline headline-listing title grid-title hlMenuAttach" onclick="return Headlines.click(event, ${hl.id});" data-article-id="${hl.id}">
										<a class="title grid-title" title="${hl.title}" target="_blank" rel="noopener noreferrer" href="${hl.link}">
											${hl.title}
										</a>
									</div>
									<div class="grid-buttons buttons-listing">
										<div class="left grid-left">
											<input dojoType="dijit.form.CheckBox" type="checkbox" onclick="Headlines.onRowChecked(this)" class='rchk'>
											<i class="marked-pic marked-${hl.id} material-icons" onclick="Headlines.toggleMark(${hl.id})">star</i>
											<i class="pub-pic pub-${hl.id} material-icons" onclick="Headlines.togglePub(${hl.id})">rss_feed</i>
										</div>
										<a href="#" class="author" onclick="Feeds.open({feed:${hl.feed_id}})">
											&mdash; ${author_display}
										</a>
										${hl.labels}
									...
									<div class="grid-content">
										${hl.content_preview.replace("&mdash; ", "")}
									</div>
								</div>
							</div>
							<div id="content-${hl.id}" style="display: none !important" class="content-inner" lang="${hl.lang ? hl.lang : 'en'}">
								<img src="${App.getInitParam('icon_indicator_white')}">
							</div>
						</div>`;
				} else if (App.getViewMode() == "cards") {
					row = `<div class="view-cards cdm ${row_class} ${Article.getScoreClass(hl.score)}" id="RROW-${hl.id}" data-article-id="${hl.id}" data-orig-feed-id="${hl.feed_id}" data-content="${escapeHtml(hl.content)}" data-score="${hl.score}" onmouseover="Article.mouseIn(${hl.id})" onmouseout="Article.mouseOut(${hl.id})">
							<div class="header">
								<div class="grid-card-container">
									<div class="grid-image">
										<center>
											<img class="img_card_preview" src="${img_preview_src}">
										</center>
									</div>
									<div style="width: 300px !important" class="grid-headline headline-listing title grid-title hlMenuAttach" onclick="return Headlines.click(event, ${hl.id});" data-article-id="${hl.id}">
										<a class="title grid-title" title="${hl.title}" target="_blank" rel="noopener noreferrer" href="${hl.link}">
											${hl.title}
										</a>
									</div>
									<div style="width: 300px !important" class="grid-buttons buttons-listing">
										<div class="left grid-left">
											<input dojoType="dijit.form.CheckBox" type="checkbox" onclick="Headlines.onRowChecked(this)" class='rchk'>
											<i class="marked-pic marked-${hl.id} material-icons" onclick="Headlines.toggleMark(${hl.id})">star</i>
											<i class="pub-pic pub-${hl.id} material-icons" onclick="Headlines.togglePub(${hl.id})">rss_feed</i>
									...
									<div style="width: 300px !important" class="grid-content">
										${hl.content_preview.replace("&mdash; ", "")}
									</div>
								</div>
							</div>
					...
					row = `<div class="view-full cdm ${row_class} ${Article.getScoreClass(hl.score)}" id="RROW-${hl.id}" data-article-id="${hl.id}" data-orig-feed-id="${hl.feed_id}" data-content="${escapeHtml(hl.content)}" data-score="${hl.score}" onmouseover="Article.mouseIn(${hl.id})" onmouseout="Article.mouseOut(${hl.id})">
							<div class="header">
								<div class="grid-full-container">
									<div class="grid-headline title grid-full-title hlMenuAttach" onclick="return Headlines.click(event, ${hl.id});" data-article-id="${hl.id}">
										<a class="title grid-full-title" title="${hl.title}" target="_blank" rel="noopener noreferrer" href="${hl.link}">
						...
						for (let i = 0; i < reply['headlines']['content'].length; i++) {
							...
							//if (App.getViewMode() == "cards") {
								...
								//if (Headlines.card_count == 1) {
									//const tmp = document.createElement("div");
									//tmp.innerHTML = "<div class='grid-cards-three'>";
									//$("headlines-frame").appendChild(tmp);
								//} else if (Headlines.card_count == 3) {
									//Headlines.card_count = 0;
									//const tmp = document.createElement("div");
									//tmp.innerHTML = "</div>";
									//$("headlines-frame").appendChild(tmp);
								//}
