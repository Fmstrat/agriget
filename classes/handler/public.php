...
class Handler_Public extends Handler {
	...
	function sharepopup() {
		...
		<html>
		<head>
			<title><?php echo __("Share with Agriget") ?> ?></title>
			<?php
		...
		if ($_SESSION["uid"]) {
			...
			if (!$feed_url) {
				...
				<form method="post">
					...
					<a href="index.php"><?php echo __("Return to Agriget") ?></a>
				</form>
				<?php
			} else {
				...
				if ($feed_urls) {
					...
					print "<a href='index.php'>".__("Return to Agriget")."</a>";
				...
				if ($feed_id) {
					...
					<a href='index.php'>".__("Return to Agriget")."</a>
					</form>";
				}
			}
			...
			<?php
				...
				if ($op == "performupdate") {
					if ($updater->isUpdateRequired()) {
						...
						print_notice("Your Agriget database is now updated to the latest version.");

						print "<a href='index.php'>".__("Return to Agriget")."</a>";

					} else {
						print_notice("Agriget database is up to date.");
					...
					if ($updater->isUpdateRequired()) {
						...
						print "<h2>".T_sprintf("Agriget database needs update to the latest version (%d to %d).",
